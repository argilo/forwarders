...
sub aliases_file
{
	defined($in{'domain'}) || error('Domain parameter is missing.');
	@domains = &list_domains();
	$number = $in{'domain'};
	if ($domains[$number] eq &get_main_domain()) {
		return [ '/etc/aliases' ];
	} else {	
		return [ '/etc/exim4/virtual/' . $domains[$number] ];
	}
}
...
print &ui_hidden("domain", $in{'domain'}),"\n";
if ($a) {
