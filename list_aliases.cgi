...
&ui_print_header(undef, "Plusendiloj por " . $domains[$in{'domain'}], "", "aliases");
...
if (@aliases) {
	...
	# render tables
	print &ui_form_start("delete_aliases.cgi?domain=$in{'domain'}", "post");
...
if ($access{'amode'} == 1 && $access{'aedit_1'} && $access{'aedit_2'} &&
	...
	foreach $f (@{&aliases_file($conf)}) {
		print "<a href='edit_file.cgi?domain=$in{'domain'}&mode=aliases&idx=$i'>",
...
&ui_print_footer("", 'list of domains');
...
foreach $a (@_) {
	local @cols;
	push(@cols, "<a href=\"edit_alias.cgi?domain=$in{'domain'}&num=$a->{'num'}\">".
